...
async function runTest() {
  ...
  // Replay them all
  const replayPromises = intentIds.map(id => 
    fetch(`${API_BASE}/replay/${id}`, { method: 'POST' }).then(res => res.json())
  );
  ...
  console.log(`Replays Executed: ${replayResults.filter((r: any) => r.success).length}`);
